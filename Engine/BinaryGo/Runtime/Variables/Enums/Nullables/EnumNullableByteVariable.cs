...
namespace BinaryGo.Runtime.Variables.Enums
{
    ...
    /// </summary>
    public class EnumNullableByteVariable<TEnum> : BaseVariable, ISerializationVariable<TEnum?>
        where TEnum : struct, Enum
    {
        /// <summary>
        /// default constructor to initialize
        /// </summary>
        public EnumNullableByteVariable() : base(typeof(TEnum?))
        {
        ...
        public void JsonSerialize(ref JsonSerializeHandler handler, ref TEnum? value)
        {
            if (value.HasValue)
            {
                ...
                handler.TextWriter.Write(Unsafe.As<TEnum, byte>(ref data).ToString(CurrentCulture).AsSpan());
#else
                handler.TextWriter.Write(Unsafe.As<TEnum, byte>(ref data).ToString(CurrentCulture));
#endif
            }
            else
            {
