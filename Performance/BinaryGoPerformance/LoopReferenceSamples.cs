...
namespace BinaryGoPerformance
{
    public class LoopReferenceSamples
    {
        public static void InitializeChaches<T>(T obj)
        {
            for (int i = 0; i < 10; i++)
            {
                BinaryGoSerializer.Serialize(obj);
        ...
        static Serializer BinaryGoSerializer { get; set; } = new Serializer(new BinaryGo.Helpers.BaseOptionInfo());

        [Benchmark]
        public void RunLoopSimpleSampleBinaryGo()
        {
            BinaryGoSerializer.Serialize(GetSimpleSample());
        }
        ...
        [Benchmark]
        public void RunLoopComeplexSampleBinaryGo()
        {
            BinaryGoSerializer.Serialize(GetComplexObjectSample());
        }
        ...
        private static void RunSample<T>(T sample, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                BinaryGoSerializer.Serialize(sample);
            }
            stopwatch.Stop();
            double BinaryGoRes = stopwatch.ElapsedTicks;

            Console.WriteLine("BinaryGo Runtime Time: \t " + stopwatch.Elapsed);
